...
function appendCellNotes(notesRoot, employee, day, assignment) {
  ...
  if ([darkCellShift, helperPlaceholderShift].includes(assignment.shift)) {
    return;
  }
...
function renderCoverage() {
  ...
  weekDays.forEach((day) => {
    ...
    const openerCoverageIssue = totals.opener > 0 ? getOpenerCoverageIssue(day) : null;
    const openerWarning = openerCoverageIssue
      ? `<div class="coverage-warning">${getOpenerCoverageWarningMessage(openerCoverageIssue)}</div>`
      : "";
...
function renderEmployeeRoster() {
  ...
  employees.forEach((employee) => {
    ...
    card.innerHTML = `
      <strong>${employee.name}</strong>
      <p>${employee.assistRow ? "Helper Row • Dark by default" : `${employee.keyType} Key • ID ${employee.idNumber || "N/A"} • Drawer ${employee.drawerNumber || "N/A"}`}</p>
...
function populateShiftEditorOptions(currentShift, day, employee = null) {
  ...
  if (currentShift === helperPlaceholderShift) {
    ...
    blankOption.value = helperPlaceholderShift;
