...
function bindEvents() {
  ...
  exportScheduleJsonButton.addEventListener("click", () => {
    exportSchedule(getSerializableState());
  });
  ...
  resetScheduleButton.addEventListener("click", async () => {
    const accepted = await askForConfirmation({
      ...
      message: "This rebuilds the schedule from the current employees, Week A/B rotation, and fixed days off. Notes, cell colors, and task selections will be cleared.",
...
function renderSaveStatus() {
  ...
  parts.push(`Loaded: ${state.loadedSaveName || "Autosaved Draft"}`);
...
function renderCellEditor() {
  ...
  populateShiftEditorOptions(assignment.shift, day);
...
function renderBulkEditor() {
  ...
  populateShiftEditorOptions("", getSelectionHasSaturdayOnly() ? "Saturday" : "Monday");
...
function generateDefaultWeekPlan(employee) {
  ...
  const weekdayShiftOptions = shiftPresets.filter(
    (shift) => !["OFF", "PTO", customShiftValue, saturdayShift].includes(shift)
  );
  ...
  weekDays.forEach((day) => {
    ...
    if (worksSaturday && day === employee.fixedDayOff) {
      plan[day] = "OFF";
      return;
    }
...
function startCellEditorDrag(event) {
  ...
  cellEditorHeader.addEventListener("pointermove", onCellEditorDrag);
  cellEditorHeader.addEventListener("pointerup", endCellEditorDrag);
  cellEditorHeader.addEventListener("pointercancel", endCellEditorDrag);
}

function onCellEditorDrag(event) {
  if (!cellEditorDragState) {
    return;
  }
...
function endCellEditorDrag(event) {
  ...
  const previewDock = cellEditorDragState.previewDock;
  ...
  cellEditorHeader.removeEventListener("pointermove", onCellEditorDrag);
  cellEditorHeader.removeEventListener("pointerup", endCellEditorDrag);
  cellEditorHeader.removeEventListener("pointercancel", endCellEditorDrag);
...
function deleteNamedSchedule(id) {
  ...
  try {
    ...
    if (state.loadedSaveId === id) {
      state.loadedSaveId = "";
      state.loadedSaveName = "Autosaved Draft";
    }
    renderSaveStatus();
...
function normalizeLoadedSchedule(raw) {
  ...
  normalizedEmployees.forEach((employee) => {
    ...
    weekDays.forEach((day) => {
      normalizedAssignments[employeeUid][day] = normalizeAssignment(rawAssignments[employeeUid]?.[day], "OFF");
    });
  });
  ...
  return {
    ...
    version: wrapper.version || payload.version || 1,
    payload: {
      version: payload.version || wrapper.version || 1,
      currentWeekType: ["Week A", "Week B", "Custom"].includes(payload.currentWeekType)
...
function applyLoadedData(normalized, options = {}) {
  ...
  state.loadedSaveName = options.loadedName || normalized.name || "Autosaved Draft";
...
function openSaveScheduleModal() {
  saveScheduleNameInput.value = state.loadedSaveName && state.loadedSaveName !== "Autosaved Draft"
