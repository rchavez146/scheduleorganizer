...
// These overrides keep the known branch roster aligned with the supervisor's
// rules even when older local saves are loaded.
const supervisorEmployeeOverrides = {
  "Mike Sedillo": {
    ...
    targetHours: 40,
    saturdayGroup: 1,
    saturdayRule: createSaturdayRuleForOffGroup(1, "Saturday off with Rose and Matthew"),
  },
  "Storm Anaya-Gonzales": {
    ...
    targetHours: 40,
    saturdayGroup: 2,
    saturdayRule: createSaturdayRuleForOffGroup(2, "Saturday off with Santiago and Christany"),
  },
  "Valentina Aragon": {
    fixedDayOff: "Friday",
    employmentType: "full_time",
    ...
    latestShiftEnd: "5:00",
    saturdayGroup: 3,
    saturdayRule: createSaturdayRuleForOffGroup(3, "Saturday off with Rodrigo and Laura"),
  },
  "Joaquin Garcia": {
    fixedDayOff: "Monday",
    employmentType: "full_time",
    targetHours: 40,
    saturdayGroup: 0,
    saturdayRule: createSaturdayRule(["Week A", "Week B", "Week C"], "Works every rotating Saturday"),
  },
  "Rudy Yanez": {
    fixedDayOff: "Thursday",
  ...
  "Rose Manassara": {
    ...
    targetHours: 40,
    saturdayGroup: 1,
    saturdayRule: createSaturdayRuleForOffGroup(1, "Saturday off with Mike and Matthew"),
  },
  "Santiago Martinez": {
    fixedDayOff: "Thursday",
    employmentType: "full_time",
    targetHours: 40,
    saturdayGroup: 2,
    saturdayRule: createSaturdayRuleForOffGroup(2, "Saturday off with Storm and Christany"),
  },
  "Rodrigo Chavez": {
    fixedDayOff: "Wednesday",
    employmentType: "full_time",
    targetHours: 40,
    saturdayGroup: 3,
    saturdayRule: createSaturdayRuleForOffGroup(3, "Saturday off with Valentina and Laura"),
  },
  "Matthew Gutierrez": {
    fixedDayOff: "Friday",
    employmentType: "full_time",
    targetHours: 40,
    saturdayGroup: 1,
    saturdayRule: createSaturdayRuleForOffGroup(1, "Saturday off with Mike and Rose"),
  },
  "Christany Silva": {
    fixedDayOff: "Tuesday",
    employmentType: "full_time",
    targetHours: 40,
    saturdayGroup: 2,
    saturdayRule: createSaturdayRuleForOffGroup(2, "Saturday off with Storm and Santiago"),
  },
  "Laura Witkoffsky Gutierrez": {
    ...
    targetHours: 40,
    saturdayGroup: 3,
    saturdayRule: createSaturdayRuleForOffGroup(3, "Saturday off with Valentina and Rodrigo"),
  },
};

// Default roster used for a fresh browser/profile or after legacy migration.
...
function bindEvents() {
  ...
  cellEditorCustomShiftButton.addEventListener("click", async () => {
    ...
    if (isInactiveShift(selected.assignment.shift)) {
      selected.assignment.tasks = createEmptyTaskState();
      selected.assignment.customTasks = [];
      selected.assignment.cellColor =
        selected.assignment.shift === darkCellShift ? darkCellColor : "";
    }
    if (selected.assignment.shift !== darkCellShift && selected.assignment.cellColor === darkCellColor) {
      selected.assignment.cellColor = "";
    }
...
// Inactive shifts do not count toward hours or staffing coverage.
function isInactiveShift(shift) {
  return ["OFF", "PTO", darkCellShift, helperPlaceholderShift].includes(shift);
}

function getDisplayShiftLabel(shift) {
  return [darkCellShift, helperPlaceholderShift].includes(shift) ? "" : shift;
}
...
function renderCoverage() {
  ...
  weekDays.forEach((day) => {
    ...
    card.innerHTML = `
      ...
      <div>Closing Lead: ${totals.closingLead}</div>
...
function buildDefaultAssignments({ preserveAssignments = true } = {}) {
  ...
  employees.forEach((employee) => {
    ...
    weekDays.forEach((day) => {
      ...
      if (day === "Saturday" && !["OFF", saturdayShift, "PTO", darkCellShift].includes(nextAssignments[employeeUid][day].shift)) {
...
function assignAutoCoverageTasksForDay(day, taskCounts) {
  ...
  const nightDropCandidates = [...scheduledEmployees]
    .filter(({ employee }) => !employee.assistRow && !["Mike Sedillo", "Storm Anaya-Gonzales"].includes(employee.name))
    .sort((left, right) => {
      const leftUid = getEmployeeUid(left.employee);
      const rightUid = getEmployeeUid(right.employee);
...
function getCoverageOpeningTime(day) {
  if (day === "Saturday") {
    return parseShiftTime("8:30");
  }

  const earlyOpeningTime = parseShiftTime("7:30");
  if (
    earlyOpeningTime !== null &&
    employees.some((employee) => {
      const employeeUid = getEmployeeUid(employee);
      const shift = state.assignments?.[employeeUid]?.[day]?.shift || "OFF";
      return getShiftStart(shift) === earlyOpeningTime;
    })
  ) {
    return earlyOpeningTime;
  }

  return parseShiftTime("8:00");
}
...
function populateShiftEditorOptions(currentShift, day, employee = null) {
  ...
  const optionGroups = [
    ...
    {
      label: "Status",
      options: [saturdayShift, darkCellShift, "OFF", "PTO"],
    },
  ];
...
function getAvailableWeekdayShifts(employee) {
  ...
  return shifts.filter((shift, index) => shifts.indexOf(shift) === index).filter((shift) => {
    if (["OFF", "PTO", darkCellShift, helperPlaceholderShift, customShiftValue, saturdayShift].includes(shift)) {
      return false;
    }
